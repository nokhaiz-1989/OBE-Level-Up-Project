...
def game_feedback(kind, amount=0, message=""):
    """Show a short animated game-style feedback popup."""
    if kind == "correct":
        icon = "✓"
        title = "CORRECT!"
        detail = f"+{amount} 🪙"
    ...
    components.html(
        ...
        <style>
          ...
          .feedback-pop {{
            min-width:240px; padding:12px 24px; border-radius:18px;
            text-align:center; background:white;
            box-shadow:0 10px 30px rgba(0,0,0,.20);
            animation: popIn .35s ease-out, floatUp 1.4s ease-in-out;
          }}
          .feedback-pop.correct {{ border:4px solid #40C057; }}
          .feedback-pop.wrong {{ border:4px solid #F03E3E; }}
          .feedback-icon {{ font-size:42px; font-weight:900; line-height:1; }}
          .correct .feedback-icon {{ color:#40C057; }}
          .wrong .feedback-icon {{ color:#F03E3E; }}
          .feedback-title {{ font-size:22px; font-weight:900; margin-top:4px; }}
          .feedback-detail {{ font-size:20px; font-weight:800; margin-top:3px; }}
          .feedback-message {{ font-size:13px; color:#666; margin-top:3px; }}
          @keyframes popIn {{ from {{ transform:scale(.45); opacity:0; }} to {{ transform:scale(1); opacity:1; }} }}
...
# Plain-language OBE journey shown to teams as they progress.
OBE_JOURNEY = [
    ("1", "IDENTIFY", "Identified the Learning Need", "🔎"),
    ("2", "DEFINE", "Defined the Learning Outcome", "🎯"),
    ("3", "DESIGN", "Designed the Learning Activity", "🧩"),
    ("4", "ALIGN", "Aligned the Assessment & Activity", "🔗"),
    ("5", "REFINE", "Refined as a Teacher", "✨"),
]


def render_obe_journey(t):
    """Show the team's OBE journey, with completed stages clearly marked."""
    completed_count = sum(t["completed"])
    cards = []

    for number, short_name, description, icon in OBE_JOURNEY:
        idx = int(number) - 1
        is_done = t["completed"][idx]
        is_current = t["level"] == int(number)
        ...
        cards.append(
            f"""
            <div class="journey-card" style="background:{bg};border-color:{border};">
                <div class="journey-top">
                    <span class="journey-icon">{icon}</span>
                    <span class="journey-number">{number}</span>
                </div>
                <div class="journey-name" style="color:{text_color};">{short_name}</div>
                <div class="journey-desc" style="color:{text_color};">{description}</div>
                <div class="journey-status" style="color:{status_color};">{status}</div>
            </div>
            """
        )
...
def team_game():
    ...
    # Game-style HUD and larger, projector-friendly typography.
    st.markdown(
        """
        <style>
        .block-container { padding-top: 1.4rem; }
        .game-title { font-size: 38px; font-weight: 900; text-align:center; margin-bottom:2px; }
        .game-subtitle { text-align:center; color:#666; font-size:17px; margin-bottom:14px; }
        .hud { display:flex; gap:12px; justify-content:center; margin:8px 0 18px; flex-wrap:wrap; }
        .hud-card { min-width:150px; padding:10px 18px; border-radius:16px; background:#f8f9fa;
                     border:2px solid #dee2e6; text-align:center; box-shadow:0 4px 10px rgba(0,0,0,.08); }
        .hud-label { font-size:13px; font-weight:800; color:#666; text-transform:uppercase; letter-spacing:1px; }
        .hud-value { font-size:27px; font-weight:900; margin-top:2px; }
        .coin-value { color:#F59F00; }
        .level-value { color:#4C6EF5; }
        [data-testid="stRadio"] label p { font-size:21px !important; line-height:1.35 !important; }
        [data-testid="stRadio"] label { padding:8px 6px; }
        [data-testid="stButton"] button { font-size:18px !important; font-weight:800 !important; min-height:48px; border-radius:12px; }
    ...
    st.markdown(f"<div class='game-title'>🏗️ {t['name']}</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='game-subtitle'>Team {team_id} • Build the Pyramid. Align the Learning.</div>", unsafe_allow_html=True)
    st.markdown(
        f"""<div class='hud'>
            <div class='hud-card'><div class='hud-label'>Level</div><div class='hud-value level-value'>{min(level,5)} / 5</div></div>
            <div class='hud-card'><div class='hud-label'>Wallet</div><div class='hud-value coin-value'>🪙 {t['coins']}</div></div>
            <div class='hud-card'><div class='hud-label'>Points</div><div class='hud-value'>⭐ {t['score']}</div></div>
        </div>""",
        unsafe_allow_html=True,
    )

    render_obe_journey(t)
    ...
    if level == 5:
        ...
        st.markdown(f"<div class='challenge-title'>{QUESTIONS[level]['title']}</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='question-box'>{QUESTIONS[level]['text']}</div>", unsafe_allow_html=True)
    ...
    if level in [1, 2, 4]:
        ...
        if st.button(
            ...
            if actual_choice == QUESTIONS[level]["answer"]:
                ...
                t["coins"] += CORRECT_REWARD
                advance(team_id, level)
                game_feedback("correct", CORRECT_REWARD, "Challenge cleared — next level unlocked!")
                ...
                game_feedback("wrong", WRONG_PENALTY, "Discuss it and try again.")
        ...
        if st.button(
            ...
            if selected == QUESTIONS[3]["answer"]:
                ...
                t["coins"] += CORRECT_REWARD
                advance(team_id, 3)
                game_feedback("correct", CORRECT_REWARD, "Perfect sequence — Level 4 unlocked!")
                ...
                t["coins"] = max(0, t["coins"] - WRONG_PENALTY)
                game_feedback("wrong", WRONG_PENALTY, "Reconsider the movement from support toward independence.")
            ...
            if b2.button(
                ...
                if match:
                    ...
                    if len(found) == len(WORD_SEARCH_WORDS):
                        ...
                        t["coins"] += CORRECT_REWARD
                        advance(team_id, 5)
                        game_feedback("correct", CORRECT_REWARD, "All hidden words found — pyramid complete!")
                    ...
                    t["coins"] = max(0, t["coins"] - WRONG_PENALTY)
                    game_feedback("wrong", WRONG_PENALTY, "Not one of the target words — try again.")
    ...
    for i, col in enumerate(cols, start=1):
        ...
        with col:
            ...
            st.markdown(
                ...
                f"{done}/5 levels • {t['score']} pts • 🪙 {t['coins']} coins</div>",
                unsafe_allow_html=True
            )
